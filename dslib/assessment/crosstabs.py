...
class CrossTabs(object):
    ...
    def sql(self):
        ...
        for name, universe in self.universes.items():
            ...
            query_str = "SELECT a.* FROM ("
            query_str += " UNION ALL ".join([f"SELECT * FROM ({self.tabs[name][str(split)].sql}) {self.tabs[name][str(split)].splits_name}" for split in self.splits])
            query_str += f") a ORDER BY {self.by_sql};"

            sql[name] = text(query_str)
